...
education_groups <- read_rds("data/value_grouped.rds")
...
ct_blocks_to_use <- blocks("CT", year = 2020) %>%
  select(GEOID20, HOUSING20) %>%
  left_join(ct_blocks_joined, by = "GEOID20") %>%
  mutate(GEOID20 = GEOID22) %>%
  select(GEOID20, HOUSING20)
  

state_dots <- furrr::future_map(state_codes, ~{
...
tippecanoe(state_dots_combined, 
           output = "data/value_dots_2022_acs.mbtiles", 
           layer_name = "value_dots_2022_acs",
           max_zoom = 12,
           min_zoom = 3,
           drop_rate = 2)

upload_tiles("data/value_dots_2022_acs.mbtiles", 
             username = "kwalkertcu",
             tileset_name = "value_dots_2022_acs")
